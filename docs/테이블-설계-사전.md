...
LH 링크를 가진 공고버전 중 아직 보충 루트가 없는 버전을 대상으로 한다. `detail_url`에서 요청 키를 읽고 공급유형으로 `SPL_INF_TP_CD`를 결정한 뒤, 공고 하나마다 `REQUIRES_NEW` 트랜잭션으로 루트와 네 자식 목록을 함께 저장한다. 이미 루트가 있으면 재호출하지 않고 unchanged로 끝낸다. 저장된 루트에는 자식을 더할 수 없도록 `requireNew()`가 막으므로 aggregate 전체가 불변이다.
...
LH 링크를 가진 공고버전 중 아직 공급 배치가 없는 버전을 대상으로 한다. 15057999와 같은 방식으로 `detail_url`과 공급유형에서 요청을 만들고 HTTP 응답을 받은 뒤, aggregate 저장만 공고 하나마다 독립 `REQUIRES_NEW` 트랜잭션으로 수행한다. 이미 배치가 있으면 재호출하지 않고 unchanged로 끝낸다. 저장된 배치에는 자식을 더할 수 없도록 `requireNew()`가 막아 aggregate를 불변으로 유지한다.
