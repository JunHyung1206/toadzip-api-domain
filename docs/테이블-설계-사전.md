...
**돈은 안 나온다 — 2026-08-14 전국 적재로 확정했다.** 응답에 `LS_GMY`(임대보증금)·`RFE`(월임대료)가
있고 API 스스로 같은 응답의 `dsList01Nm`에 "임대보증금(원)", "월임대료(원)"이라고 쓴다. 그런데
**주택형 공급행 256행 전부가 문자열 `"공고문 참조"`였다. 숫자는 0건이다.**
...
| LH 15056765 | `LS_GMY`·`RFE` | **0 / 256** (전부 `"공고문 참조"`) | 공고 × 단지 × **주택형** ← 알갱이는 여기만 맞음 |
...
| 항목 | 값 |
| --- | --- |
| `housing_complex` | **3,286** (물리 단지 2,969 × 공급유형) |
| `unit_type` | 11,170 |
| `notice` | 52 (원공고 43 + 정정 9) |
| `notice_supply` | **271** — 주택형 단위 256 + 단지 단위 15 |
| 주택형 확정(`unit_type_id`) | 198 / 271 |
| 미확정 — 주소가 안 맞아 마이홈 행에 못 붙음 | 24 |
| 미확정 — 면적 후보 다수 | 28 |
| 미확정 — PNU로 단지를 못 찾음 | 9 |
| 미확정 — LH가 이 단지의 주택형 행을 안 줌 | 11 |
| 15059475 원천행 | 6,710 (그중 카탈로그 반영 1,487) |
| `notice_schedule` / `reception_place` / `notice_attachment` | 100 / 48 / 435 |

`housing_complex`가 2,969가 아니라 3,286인 이유는 자연키가 `(hsmpSn, 공급유형명)`이기 때문이다.
한 `hsmpSn`이 공급유형 2개로 갈린 단지가 257개, 3개로 갈린 단지가 30개다. 예전 문서의 2,969는
공급유형을 자연키에 넣기 전에 적재된 DB를 센 값이라 물리 단지 수였다.

공급 호수 합계는 기준에 따라 다르다 — **LH 기준 11,513호, 마이홈 기준 11,266호**(§4.2 참고).
