...
이 문서는 프로젝트를 처음 보는 개발자가 20개 테이블의 역할과 연결 방식을 빠르게 찾기 위한 사전이다. 각 테이블은 다음 순서로 설명한다.
...
`HousingProviderAgency → HousingComplex → ComplexRentalProgram → UnitType`은 마이홈 15110581이 현재 알려 주는 공급기관·단지·공급유형·주택형을 보관한다. 같은 원천 식별자를 다시 수집하면 새 이력 대신 변경 가능한 상세값을 갱신한다.
...
| --- | --- | --- | --- | --- |
| `housing_provider_agency` | `HousingProviderAgency` | 최신 카탈로그 | 공급기관 하나 | 없음 |
| `housing_complex` | `HousingComplex` | 최신 카탈로그 | 원천 시스템 × 원천 단지 ID | `housing_provider_agency` |
| `complex_rental_program` | `ComplexRentalProgram` | 최신 카탈로그 | 단지 × 공급유형명 | `housing_complex` |
| `unit_type` | `UnitType` | 최신 카탈로그 | 프로그램 × 주택형명 × 두 면적 | `complex_rental_program` |
...
**행 하나**는 마이홈 `insttNm`으로 식별한 공급기관 하나다. 현재 안정적인 기관 코드가 없어 정규화한 기관명을 `code`와 `name`에 함께 저장한다.

단지마다 반복되는 기관명을 한 번만 보관하려고 분리했다. 따라서 `code`는 공식 기관 코드가 아니라 현재의 이름 기반 식별자다.

**관계/키:** PK `id` · UNIQUE `code` · 자식 `housing_complex`

| 속성(DB 컬럼) | 의미 | 출처/비고 |
| --- | --- | --- |
| `id` | 내부 기관 PK | DB 생성 |
| `code` | 기관 조회 식별값 | 15110581 `insttNm`; 현재 `name`과 같음 |
| `name` | 공급기관명 | 15110581 `insttNm` |

### 5.2 `housing_complex`

**행 하나**는 `source_system + source_complex_id`로 식별되는 현재 단지 하나다. 이름이나 주소가 아니라 마이홈 `hsmpSn`이 원천 정체성이다.

공급유형과 주택형마다 반복되는 주소·준공·난방 같은 단지 속성을 모으기 위해 분리했다. 이 행은 최신 카탈로그이며 공고 당시 상태는 `lh_complex_detail`에서 찾는다.

**관계/키:** PK `id` · FK `housing_provider_agency_id` · UNIQUE `(source_system, source_complex_id)` · 자식 `complex_rental_program`
...
| `housing_provider_agency_id` | 공급기관 FK | `insttNm`으로 찾거나 만든 기관 |

### 5.3 `complex_rental_program`

**행 하나**는 한 단지가 한 원천 공급유형명으로 운영하는 임대 프로그램 하나다. `unit_count`도 단지 전체가 아니라 이 단지·공급유형의 세대수다.

한 단지에 국민임대와 행복주택이 함께 있을 수 있고 세대수와 임대조건의 문맥이 달라서 단지와 주택형 사이에 두었다.

**관계/키:** PK `id` · FK `housing_complex_id` · UNIQUE `(housing_complex_id, supply_type_name)` · 자식 `unit_type`

| 속성(DB 컬럼) | 의미 | 출처/비고 |
| --- | --- | --- |
| `id` | 내부 프로그램 PK | DB 생성 |
| `supply_type` | 표준 공급유형 | 15110581 `suplyTyNm` 변환 |
| `supply_type_name` | 공급유형 원문 | 15110581 `suplyTyNm` |
| `unit_count` | 단지·공급유형 전체 세대수 | 15110581 `hshldCo` |
| `housing_complex_id` | 소속 단지 FK | 같은 `hsmpSn`의 단지 |

### 5.4 `unit_type`

**행 하나**는 한 임대 프로그램 안에서 주택형명·전용면적·주거공용면적이 같은 카탈로그 주택형 하나다.

주택형명만으로 다른 면적을 구분할 수 없고, 같은 평면도 공급유형에 따라 기본 임대조건이 달라 부모 프로그램 아래 분리했다.

**관계/키:** PK `id` · FK `complex_rental_program_id` · UNIQUE `(complex_rental_program_id, type_name, exclusive_area, residential_common_area)`
...
| `complex_rental_program_id` | 소속 프로그램 FK | 단지와 공급유형으로 찾은 프로그램 |
...
| `lh_lease_info.complex_total_unit_count` | 단지·공급유형 전체 세대수 | `SUM_HSH_CNT`; 프로그램 세대수 검증 |
...
`SUM_HSH_CNT`가 카탈로그 프로그램 하나로 좁혀지고 `DDO_AR`가 `UnitType.exclusive_area`와
...
| `candidate_count` | 해당 단계 후보 수 | 프로그램·면적 후보 또는 중복 원천행 수 |
...
4. **LH 카탈로그 matcher:** 15059475의 지역·단지명·공급유형·`SUM_HSH_CNT`를 카탈로그 프로그램과 비교한 뒤, `DDO_AR`가 정확히 같은 `UnitType`에 `HSH_CNT`를 반영한다.
...
| LH 카탈로그 | `MATCHED` | `AMBIGUOUS`: 프로그램 또는 원천행 중복, `UNMATCHED`: 정확한 면적 후보 없음, `CONFLICT_PROGRAM_UNIT_COUNT`: `SUM_HSH_CNT` 충돌 |
...
이 값은 `ComplexRentalProgram.unit_count(SUM_HSH_CNT)`와 다르며, 공고의 이번 회차 공급 수와도 다르다.
