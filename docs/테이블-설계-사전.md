...
| `candidateLhComplexDetailIds` | `candidate_lh_complex_detail_ids` | 일반 공고 평가 행에서는 후보 `LhComplexDetail.id`를 정렬해 쉼표로 이은 문자열; `notice_housing_id=NULL`인 LH 잔여 `UNMATCHED` 행에서는 그 잔여 상세 자신의 ID |
...
matcher는 먼저 해당 공고버전의 `LhNoticeSupplement`를 조회한다. 보충 aggregate 자체가 없으면 즉시 반환해 결과 행을 하나도 만들지 않는다. 보충 aggregate는 있지만 `dsSbd` 데이터셋 키가 없을 때만 공고 공급행마다 `SOURCE_DETAIL_MISSING`을 남긴다. 이 둘은 “보충 수집 자체가 없음”과 “응답은 수집했지만 단지 상세 데이터셋이 없음”이라는 서로 다른 상태다.

데이터셋이 있으면 양방향 유일 후보에서 세대수가 둘 다 있고 같을 때 `MATCHED_ADDRESS_AND_UNIT_COUNT`, 한쪽 세대수가 없으면 `MATCHED_ADDRESS_ONLY`, 둘 다 있지만 다르면 `CONFLICT_UNIT_COUNT`다. 공고 쪽 후보가 없으면 `UNMATCHED`, 한쪽 방향에서 후보가 여러 건이면 `AMBIGUOUS`다. 데이터셋은 있지만 단독 후보로 소비되지 않은 LH 상세는 `notice_housing_id=NULL`인 `UNMATCHED` 잔여 행으로 남긴다.
...
| `candidate_lh_complex_detail_ids` | `NoticeHousingLhMatch.evidence.candidateLhComplexDetailIds` | `varchar(500)` | 허용 | 일반 공고 평가 행은 후보 LH 상세 ID를 정렬한 쉼표 목록; `notice_housing_id=NULL`인 LH 잔여 `UNMATCHED` 행은 후보 목록이 아니라 그 잔여 상세 자신의 ID를 식별 근거로 저장 | 후보 `LhComplexDetail.id` 목록 또는 잔여 `LhComplexDetail.id`; `NoticeHousingLhMatchEvidence` 평탄화 |
...
- 결과 행이 아예 없다는 사실만으로 `UNMATCHED`라고 판정하면 안 된다. matcher가 아직 실행되지 않았거나, 실행했어도 `LhNoticeSupplement` 자체가 없어 즉시 반환한 경우가 모두 0행으로 보인다. 보충 수집 여부와 matcher 실행 이력을 별도로 확인해야 한다.
...
| `NoticeHousingLhMatchStatus` | `SOURCE_DETAIL_MISSING` | `LhNoticeSupplement`는 존재하지만 15057999 응답에 LH 단지 상세 `dsSbd` 데이터셋 키가 없음; 보충 aggregate 자체가 없으면 이 상태 행도 만들지 않음 |
...
선택한 matcher 버전의 LH 매칭 행이 없을 때도 이를 곧바로 `UNMATCHED`로 표시하지 않는다. matcher를 아직 실행하지 않은 경우와, 실행했지만 `LhNoticeSupplement` 자체가 없어 0행으로 반환한 경우가 같은 모습이다. 먼저 `lh_notice_supplement` 존재 여부를 확인하고, matcher 실행 여부는 호출 기록이나 운영 상태로 별도 판별한다. supplement가 존재하면서 `complex_detail_dataset_present=false`인 경우에는 비로소 공고 공급행별 `SOURCE_DETAIL_MISSING` 결과가 있어야 한다.
...
다음은 별도 저장소 스크립트 없이 같은 비교를 재현하는 shell recipe다. H2 조회는 기존 파일만 여는 절대 `IFEXISTS=TRUE` URL을 사용하고, 비교용 CSV만 `/tmp`에 쓴다.

```bash
DOC_PATH='docs/테이블-설계-사전.md'
H2_CP=$(find /Users/pjh/.gradle/caches/modules-2/files-2.1/com.h2database/h2 \
  -path '*/h2-2.4.240.jar' -print -quit)
DB_URL='jdbc:h2:file:/Users/pjh/source/domain/data/domain-construction-rental-v2;IFEXISTS=TRUE;MODE=MySQL;AUTO_SERVER=TRUE'

# 상세 절의 "전체 컬럼" 표에서만 table,column 쌍을 추출한다.
awk '
/^### [0-9]+\.[0-9]+ `[^`]+`$/ {
  split($0, a, "`"); table=a[2]; columns=0; next
}
/^#### 전체 컬럼$/ { columns=1; next }
/^#### / { columns=0 }
columns && /^\| `/ {
  split($0, a, "`"); printf "\"%s\",\"%s\"\n", table, a[2]
}
' "$DOC_PATH" | sort > /tmp/table-doc-documented-columns.csv

# PUBLIC 스키마의 실제 persisted column 쌍을 같은 형식으로 출력한다.
java -cp "$H2_CP" org.h2.tools.Shell -url "$DB_URL" -user sa -password '' -sql \
"CALL CSVWRITE('/tmp/table-doc-db-columns.csv',
 'SELECT LOWER(TABLE_NAME), LOWER(COLUMN_NAME)
    FROM INFORMATION_SCHEMA.COLUMNS
   WHERE TABLE_SCHEMA=''PUBLIC''
   ORDER BY TABLE_NAME, ORDINAL_POSITION',
 'charset=UTF-8 fieldSeparator=, writeColumnHeader=false');"

sort /tmp/table-doc-db-columns.csv -o /tmp/table-doc-db-columns.csv
diff -u /tmp/table-doc-documented-columns.csv /tmp/table-doc-db-columns.csv
wc -l /tmp/table-doc-documented-columns.csv /tmp/table-doc-db-columns.csv
cut -d, -f1 /tmp/table-doc-documented-columns.csv | sort -u | wc -l

# 각 상세 절의 키 표에서 table,type,column-list를 추출한다.
awk '
/^### [0-9]+\.[0-9]+ `[^`]+`$/ {
  split($0, a, "`"); table=a[2]; keys=0; next
}
/^#### 키와 제약$/ { keys=1; next }
/^#### / { keys=0 }
keys && /^\| (PK|FK|UNIQUE) \| `/ {
  split($0, a, "`"); columns=a[2]; sub(/ →.*/, "", columns); gsub(/ /, "", columns)
  split($0, f, " "); printf "\"%s\",\"%s\",\"%s\"\n", table, f[2], columns
}
' "$DOC_PATH" | sort > /tmp/table-doc-documented-constraints.csv

# 실제 복합키도 ordinal 순서로 묶어 문서의 키 컬럼 조합과 비교한다.
java -cp "$H2_CP" org.h2.tools.Shell -url "$DB_URL" -user sa -password '' -sql \
"CALL CSVWRITE('/tmp/table-doc-db-constraints.csv',
 'SELECT LOWER(TC.TABLE_NAME),
         CASE TC.CONSTRAINT_TYPE
           WHEN ''PRIMARY KEY'' THEN ''PK''
           WHEN ''FOREIGN KEY'' THEN ''FK''
           ELSE ''UNIQUE''
         END,
         LISTAGG(LOWER(KCU.COLUMN_NAME), '','')
...
sort /tmp/table-doc-db-constraints.csv -o /tmp/table-doc-db-constraints.csv
diff -u /tmp/table-doc-documented-constraints.csv /tmp/table-doc-db-constraints.csv
awk -F, '{ gsub(/"/, "", $2); count[$2]++ }
END { for (kind in count) print kind, count[kind] }' \
  /tmp/table-doc-db-constraints.csv | sort

# 22개 FK의 양쪽 규칙이 모두 NO ACTION인지 별도로 확인한다.
java -cp "$H2_CP" org.h2.tools.Shell -url "$DB_URL" -user sa -password '' -sql \
"SELECT DELETE_RULE, UPDATE_RULE, COUNT(*)
   FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS
  GROUP BY DELETE_RULE, UPDATE_RULE;"
```

성공 기준은 두 `diff`가 출력 없이 종료하고, 컬럼 CSV 두 파일이 각각 190행, 문서 테이블명이 17개이며, 마지막 제약 집계가 `PK 17`, `FK 22`, `UNIQUE 18`을 출력하는 것이다. FK 규칙 쿼리는 `NO ACTION | NO ACTION | 22` 한 행이어야 한다.
