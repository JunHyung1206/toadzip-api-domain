...
수치는 전부 `data/domain-construction-rental-v3`(2026-08-13 전국 적재)를 직접 센 것이다.
...
**금회공급호수는 확실히 나온다.** `NOW_HSH_CNT`가 그것이고, 적재된 290행 전부에 값이 있다(합계 13,575호).
매칭 없이 원천이 그대로 준다.

**돈은 "필드는 있는데 숫자라는 보장이 없다."** 응답에 `LS_GMY`(임대보증금)·`RFE`(월임대료)가 있고
API 스스로 같은 응답의 `dsList01Nm`에 "임대보증금(원)", "월임대료(원)"이라고 쓴다. 그런데 기록해 둔
실측 fixture에서는 두 값이 문자열 `"공고문 참조"`였다.
...
숫자 컬럼은 아직 만들지 않았다 — 숫자가 온다는 걸 확인하기 전에 파싱 컬럼부터 만드는 건 추측이다.
한 번 적재해서 `"공고문 참조"`가 아닌 값이 몇 건인지 세면 답이 나온다.

돈이 확실히 나오는 곳은 따로다.

| 원천 | 돈 | 채워진 비율 | 알갱이 |
| --- | --- | --- | --- |
| 마이홈 15108420 | 보증금·계약금·잔금·월임대료 | **113 / 113** | 공고 × **단지** (주택형별 아님) |
| LH 15059475 | `LS_GMY`·`RFE` | **6,710 / 6,710** 전부 숫자 | 현재 카탈로그 (**공고 시점 아님**) |
| LH 15056765 | `LS_GMY`·`RFE` | 미확인 | 공고 × 단지 × **주택형** ← 알갱이는 여기만 맞음 |

즉 **"이번 공고의, 주택형별, 실제 임대료"를 확실히 주는 원천은 아직 없다.**
...
| `notice_supply` | `NoticeSupply` | 공고 × 단지 × 주택형 | `notice` |
...
**사라진 11개:** `recruitment_notice`, `notice_version`(→`notice`), `notice_housing`,
...
| `unit_type_id` | 카탈로그 주택형 FK | 전용면적 ±0.05㎡ 유일 후보만 |
...
| `supplied_address` | 전체 주소 | `fullAdres`; LH `dsSbd` 지번주소와 대조하는 키 |
...
**(1) 같은 단지가 두 줄로 남을 수 있다.** 2026-08-13 실측에서

- LH 주택형 행 290개 중 **37개**(2,404호)가 주소 매칭이 끊겨 마이홈 공급행을 못 만났다 → `house_sn`·돈이 `NULL`
- 마이홈 공급행 113개 중 **20개**(2,605호)가 LH 주택형 행을 못 만났다 → `type_name`·`unit_supply_count`가 `NULL`

합치면 `notice_supply`는 **310행**이 되는데, 이 37과 20 중 일부는 주소가 안 맞아 서로를 못 찾은
...
```sql
-- LH 기준 (주택형까지 쪼개진 것만)
SELECT SUM(unit_supply_count) FROM notice_supply WHERE type_name IS NOT NULL;      -- 13,575

-- 마이홈 기준 (단지 단위, house_sn 중복 제거)
SELECT SUM(complex_supply_count) FROM (SELECT DISTINCT notice_id, house_sn, complex_supply_count
                                       FROM notice_supply WHERE house_sn IS NOT NULL);  -- 12,927
```
...
`NO_CATALOG_PATH`(주소·PNU 구간에서 끊김)가 둘 다 `unit_type_id IS NULL`이 된다. 실측이
`MATCHED` 216 / `NO_CATALOG_PATH` 44 / `AMBIGUOUS` 30이었으니 74건의 원인이 통째로 사라지는데,
그게 규칙을 고칠 유일한 근거라 `unmatched_reason` 한 칸으로 남겼다. 후보 수·matcher 버전·판정 시각은
안 남긴다 — 그건 match 테이블을 되살리는 것이다.
...
2. **건설임대 필터.** `SupplyType.isConstructionRental()`이 하던 일은 enum 없이도 된다. 저장이 아니라
   적재 시점 필터라서, 허용 이름 8개 집합으로 거르면 그만이다(`ConstructionRentalPolicy`).
...
아래는 **이전 18개 테이블 구조로 수행한 전국 적재의 관측값**이며 스키마의 고정값이 아니다.
...
| `unit_type` | 11,170 |
| 공고(구 `notice_version`) | 58 (그중 LH 56, 지방공사 2) |
| 마이홈 공급행(구 `notice_housing`) | 113 |
| LH 주택형 공급행(구 `lh_unit_supply`) | 290 |
| 주택형 확정 | 216 |
| 주택형 미확정 — 면적 후보 다수 | 30 |
| 주택형 미확정 — 앞 구간 단절 | 44 |
| 15059475 원천행 | 6,710 (그중 카탈로그 반영 1,487) |
