...
`UNKNOWN_SUPPLY_TYPE`으로, 매입·전세면 `UNSUPPORTED_SUPPLY_TYPE`으로 제외한다. 라벨 판정을 통과해도
준공일이 파싱되지 않고 주택유형도 아파트가 아니면 `NOT_CONSTRUCTION_HOUSING`으로 한 번 더 거른다 —
"행복주택"이라는 라벨을 달고 있어도 실체가 매입주택인 행이 있기 때문이다.

실측: 전국 256개 시군구를 돌린 결과 단지 2,969개가 만들어졌고, 121,746건이
...
**그래서 합계는 한쪽 기준으로만 낸다.** LH 주택형 행 290개 중 37개가 마이홈 공급행을 못 만났고,
...
다르다(LH 13,575호 vs 마이홈 12,927호). [NoticeSupply.java:31](../src/main/java/test/domain/notice/NoticeSupply.java)
...
단지가 확정되면 그 단지의 `UnitType` 중 전용면적 차이가 0.05㎡ 이내인 것을 고른다. 후보가 하나일 때만
확정하고, 아니면 이유를 남긴다.
...
API는 같은 응답의 `dsList01Nm`에 "임대보증금(원)"·"월임대료(원)"이라고 스스로 써 준다. 그런데 기록해 둔
실측 fixture에서는 두 값이 `"공고문 참조"` 문자열이었다. 파싱해 버리면 어느 쪽이 왔는지 셀 수가 없다.

이 두 값은 **주택형별 임대료를 주는 유일한 원천**이다.

| 원천 | 돈 | 채워진 비율 | 알갱이 |
| --- | --- | --- | --- |
| 마이홈 15108420 | 보증금·계약금·잔금·월임대료 | 113 / 113 | 공고 × **단지** |
| LH 15059475 | `LS_GMY`·`RFE` | 6,710 / 6,710 전부 숫자 | 현재 카탈로그 (**공고 시점 아님**) |
| LH 15056765 | `LS_GMY`·`RFE` | 미확인 | 공고 × 단지 × **주택형** |

그래서 원문을 남긴 뒤, 숫자가 오는 공고가 얼마나 되는지 확인하고 숫자 컬럼으로 승격한다.
