...
2026-08-16 재적재 실측: 전국 256개 시군구를 돌린 결과 물리 단지 2,837개(공급유형별 3,038행)가
만들어졌고, 123,986건이 `UNSUPPORTED_SUPPLY_TYPE`(매입·전세·장기전세 등)으로, 80건이
`NOT_CONSTRUCTION_HOUSING`(라벨은 건설임대인데
...
`LhUnitSupply`(LH 15056765), `LhComplexDetail`(LH 15057999 `dsSbd`), 그리고 match 4개를 한 행으로
합친 자리다.

**행의 알갱이가 두 가지다.** LH가 주택형을 쪼개 주면 `공고 × 단지 × 주택형`이고, 안 주면
(SH·GH 공고, 또는 주소가 안 맞아 못 붙인 단지) `공고 × 단지` 한 줄이다. `typeName`이 있는지로 구분한다.

**그래서 합계는 한쪽 기준으로만 낸다.** 2026-08-16 재적재에서 주택형 행 252개 중 24개가 마이홈
공급행을 못 만났고, 마이홈 공급행 107개 중 21개가 주택형 행으로 안 쪼개졌다. 이 둘 중 일부는 주소가 안 맞아
서로를 못 찾은 **같은 단지**라 한 테이블에서 두 줄로 보인다. 두 원천이 말하는 총 공급 호수도 애초에
다르다(LH 11,348호 vs 마이홈 13,251호). [NoticeSupply.java:31](../src/main/java/test/domain/notice/NoticeSupply.java)
...
아니었다 — LH 단지 상세 주소는 공급행을 잇는 **중간 계산**이고, 남길 값은 입주예정월뿐이라
...
둘 다 `unit_type_id IS NULL`이 된다. 2026-08-16 재적재는 확정 224 / 앞 구간 단절 49 / 면적 모호 0이다.
지금은 면적 모호가 없지만 규칙이나 원천이 바뀌면 다시 생길 수 있고, 실패 원인이 규칙을 고칠 근거라
`unmatchedReason` 한 칸으로 남겼다.
...
전용면적 완전일치도 후보를 가르지 못하면 마지막으로 LH 공급면적(`SPL_AR`)과 카탈로그의
`전용면적 + 주거공용면적`을 ±0.05㎡로 비교한다. 산남주공2단지처럼 전용면적과 이름은 같고 공용면적만
다른 경우를 위한 규칙이며, 실측 8건을 추가로 확정했다. 그래도 유일하지 않으면 이유를 남긴다.
[NoticeSupplyCatalogLinker.java:109](../src/main/java/test/domain/ingest/NoticeSupplyCatalogLinker.java)
...
명명이라 2026-08-16 재적재의 주택형 공급행 252개가 전부 맞았다.
...
표기가 갈린다. 두 원천 다 ㎡ 소수로 주는 **전용면적**(허용오차 0.05㎡)을 주 근거로 삼고, 필요한 경우
공급면적을 마지막 동률 해소 기준으로 쓴다. 원문 주택형명은 `typeName`에 그대로 남긴다.

### 앞 두 구간은 행을 만들 때 결정된다

상세주소 매칭은 사후 재계산이 안 된다. **어느 LH 주택형 행과 어느 마이홈 공급행이 한 줄이 되는가**를
...
2026-08-16 재적재(주택형 공급행 252개): 주택형 확정 224 · 앞 구간 단절 28 · 면적 후보 다수 0 ·
단지는 맞는데 면적 없음 0. 단지만 확정되면 주택형도 100% 확정되어 남은 손실은 전부 주소·PNU 구간이다.
...
2026-08-16 재적재에서 **주택형 공급행 252행 전부가 `"공고문 참조"` 문자열이었다. 숫자는 0건이다.**
...
| 원천 | 돈 | 채워진 비율 | 알갱이 |
| --- | --- | --- | --- |
| 마이홈 15108420 | 보증금·계약금·잔금·월임대료 | 107 / 107 | 공고 × **단지** |
| LH 15059475 | `LS_GMY`·`RFE` | 6,710 / 6,710 전부 숫자 | 현재 카탈로그 (**공고 시점 아님**) |
| LH 15056765 | `LS_GMY`·`RFE` | **0 / 252** | 공고 × 단지 × **주택형** |
