...
`UnitType`은 공고와 직접 연결되지 않는다 — 5장에서 지금 진행 중인 작업을 다룬다.
...
### `NoticeHousingUnitTypeMatch` — 확정된 단지 위에서 전용면적으로 주택형과 매칭

한 `PAN_ID` 응답에 단지 여러 곳이 섞여 나와 PNU가 없다. 그래서 `NoticeHousingCatalogMatch`가 PNU로 **이미 확정한 `HousingComplex`** 위에서만 시도한다 — 단지 자체가 안 붙었으면 `NO_CATALOG_MATCH`로 남기고 더 진행하지 않는다.

```
NoticeHousing → (NoticeHousingCatalogMatch, MATCHED_PNU) → HousingComplex
    → 같은 이름의 LhUnitSupply 행들(SBD_LGO_NM == HousingComplex.name, 문자열 대조)
        → 전용면적이 허용오차(0.05㎡) 안에서 유일한 UnitType 후보면 MATCHED
```

주택형명(`HTY_NNA`)은 매칭 키로 쓰지 않는다 — "59㎡"처럼 단위가 붙어 카탈로그 `styleNm`("59")과 표기가 갈리기 때문이다. 대신 두 원천 다 ㎡ 소수로 주는 **전용면적**을 근거로 삼고, 원문 주택형명은 `sourceTypeName` 컬럼에 증거로만 남긴다. `NoticeHousing` 하나가 주택형을 여럿 공급하면(단지 안 여러 형이 이번 공고에 같이 나오면) 15056765 공급행 수만큼 매칭 결과 행이 생긴다 — `NoticeHousingLhMatch`와 달리 1:1이 아니라 1:N이다.

`NoticeHousingLhMatch`와 마찬가지로 `matcherVersion`별 재계산 가능한 파생 테이블이고, 이번 회차 공급 세대수(`NOW_HSH_CNT`)를 `suppliedUnitCount`로 남긴다. `/admin/ingest/matches/catalog`로 단지를 먼저 확정한 뒤 `/admin/ingest/matches/unit-type`을 돌린다. [NoticeHousingUnitTypeMatchService.java:59](../src/main/java/test/domain/match/NoticeHousingUnitTypeMatchService.java)
