...
셀 수가 없다. **2026-08-16 21:52 KST 전체 재적재에서 주택형 공급행 252행 전부가 `"공고문 참조"`였다 — 숫자 0건.**
...
아래 수치는 `data/domain-construction-rental-v4`를 2026-08-16 21:52 KST에 전체 재적재한 결과다. 원천이 살아 있어 공고 수는
날마다 달라진다.
...
| `unit_type` | 10,024 |
| `notice` | 52 |
| LH 상세·공급정보 수집 완료 공고 | 48 |
| `notice_supply` — 주택형 단위 | 252 |
| `notice_supply` — 단지 단위 잔여 | 21 |
| 카탈로그 `HousingComplex` 확정 연결 | 240 |
| 카탈로그 `UnitType` 확정 연결 | 224 |
| 같은 면적 후보가 여러 개라 미확정 | 0 |
| 주소가 안 맞아 마이홈 행에 못 붙음 | 24 |
| PNU로 단지를 못 찾아 미확정 | 9 |
| LH 응답에 해당 단지 주택형 행이 없음 | 12 |
| LH 주택형별 공급정보를 받지 못한 단지 단위 행 | 4 |
| 15059475 원천행 → 카탈로그 반영 | 6,710 → 1,481 |

현재는 공급면적을 마지막 동률 해소 기준으로 사용해 면적 후보 다수로 남은 행이 없다.
경로가 끊긴 건 주소·PNU·LH 원천 연결이 안 된 경우이며 LH 공급행 자체가 잘못됐다는 뜻은 아니다.
