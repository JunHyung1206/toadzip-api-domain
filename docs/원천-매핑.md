...
**수치는 `data/domain-construction-rental-v4`의 2026-08-16 21:52 KST 전체 재적재 결과를 직접 센 것이다.**

```
단지 3,038(물리 2,837 × 공급유형) · 주택형 10,024 · 공고 52 · 공급행 273(주택형 252 + 단지 21)
15059475 원천행 6,710 → 카탈로그 반영 1,481
```
...
| | 뜻 | 단지라는 게 있나 |
| --- | --- | --- |
| **건설임대** | 공공이 **직접 지었다** — 국민임대·영구임대·행복주택·통합공공임대·5·10·50년임대 | 있다 |
| 매입임대 | 민간주택을 한 채씩 사들였다 | 없다. 단지명 칸에 "경기도 수원시" 같은 지역명이 온다 |
| 전세임대 | 세입자가 직접 집을 구해온다 | 없다. 대상 주택 자체가 우리 쪽에 없다 |
| 장기전세 | SH가 재건축 조합에서 매입한 물량이 대부분이다 | 이름은 있는데 민간 단지에 몇십 세대씩 박혀 있다 |

**라벨만 믿지 않는다.** `ConstructionRentalPolicy`가 두 단계로 거른다 — 허용 이름 7개가 아니면
...
전국 적재 결과 단지 3,038행(물리 단지 2,837개)이 남고, 123,986건이 공급유형으로, 80건이 건설 증거 없음으로 걸러졌다.
...
**결과는 알갱이가 섞인 테이블이다.** 주택형 행 252개 중 24개가 마이홈 공급행을 못 만났고, 마이홈
공급행 107개 중 21개가 주택형 행으로 안 쪼개졌다. 합계를 낼 때는 반드시 한쪽 기준으로만 낸다 — LH 기준
11,348호, 마이홈 기준 13,251호다(자세한 건 [테이블-설계-사전.md §4.2](테이블-설계-사전.md)).
...
| `total_unit_count` | 15059475 `HSH_CNT`; 유일 매칭 때만 | 1,481 / 10,024 |
...
| `contact` | `refrnc` | 95% |
| `source_pan_id` · `lh_supply_info_type_code` · `lh_fetched_at` | LH 호출 흔적 | 호출 완료 48/52 |
...
15056765는 LH 전용이다. 2026-08-16 21:52 KST 적재에서 공고 52건 중 48건이 LH 응답을 받아 주택형까지
쪼개졌고, 나머지 4건은 단지 단위 행으로 남는다. 지방공사 3건과, `SPL_INF_TP_CD`를 몰라 호출하지 못한
LH 통합공공임대 1건이다. data.go.kr에 SH·GH 이름으로 찾아도 공고 단위
...
**`계약금 + 잔금 = 임대보증금`이 현재 마이홈 공급행 107건 중 101건에서 성립한다.** 어긋나는 6건은 원천 값 자체가 안 맞는다.
...
알갱이가 맞는 건 15056765의 `LS_GMY`·`RFE`뿐이었는데, 2026-08-16 21:52 KST 전체 재적재에서 **주택형 공급행
252행 전부가 `"공고문 참조"` 문자열이었다. 숫자는 0건이다.**

```sql
SELECT lh_deposit_text, COUNT(*) FROM notice_supply GROUP BY 1;
-- 공고문 참조 | 252
-- (null)    |  21   ← 15056765를 못 받거나 주택형 행으로 쪼개지지 않은 단지 단위 행
```
...
공급행을 연결할 때 행마다 `findAllByAddressPnuAndSupplyTypeName`을 부른다. 전국 단지 2,837개면 아직
...
공고 52건 중 기관명이 LH인 공고는 49건이다. 그중 호출 파라미터를 확보해 LH 상세·공급정보를 수집한 공고는 48건이고, 통합공공임대 1건은 `SPL_INF_TP_CD`가 없어 호출하지 못했다.
