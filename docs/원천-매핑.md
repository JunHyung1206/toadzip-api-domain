...
**수치는 2026-08-13 전국 적재 결과를 직접 센 것이다.**

```
단지 2,969 · 주택형 11,170 · 공고 58 · 마이홈 공급행 113 · LH 주택형 공급행 290 · 15059475 원천행 6,710
```
...
전국 적재 결과 단지 2,969개가 남고, 121,746건이 공급유형으로, 248건이 건설 증거 없음으로 걸러졌다.
...
**결과는 알갱이가 섞인 테이블이다.** LH 주택형 행 290개 중 37개가 마이홈 공급행을 못 만났고, 마이홈
공급행 113개 중 20개가 LH 주택형 행을 못 만났다. 합계를 낼 때는 반드시 한쪽 기준으로만 낸다
(자세한 건 [테이블-설계-사전.md §4.2](테이블-설계-사전.md)).
...
15056765는 LH 전용이다. 공고 58건 중 56건이 LH라 대부분 주택형까지 쪼개지지만, 강원개발공사·
경상북도개발공사 공고 2건은 단지 단위 행으로 남는다. data.go.kr에 SH·GH 이름으로 찾아도 공고 단위
...
### 5.3 주택형별 임대료는 아직 확실한 원천이 없다

마이홈은 임대조건을 **단지 단위**로만 주고, 15059475는 공고가 아니라 **현재 카탈로그 기준값**이다.
알갱이가 맞는 건 15056765의 `LS_GMY`·`RFE`뿐인데 기록해 둔 실측 fixture에서는 두 값이
`"공고문 참조"` 문자열이었다.

그래서 문자열로 받아 두고(`lh_deposit_text`, `lh_monthly_rent_text`) 숫자가 오는 공고가 얼마나 되는지
세어 본 뒤에 숫자 컬럼으로 승격한다. 지금 `notice_supply.deposit`은 단지 단위 값이 주택형 행마다
복사된 것이라 주택형별로 구분되지 않는다.
