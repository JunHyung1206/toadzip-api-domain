 ...
 *
 * <p>단지 카탈로그와의 매칭은 별도 파생 matcher 의 책임이라 여기서는 다루지 않는다. 305행 중 PNU가 있는
 * 행이 147개뿐이라 매칭이 되어도 절반은 못 붙는다. 매칭되는 경우에도 이건 지우지 않는다. 공고버전이
 * 불변 스냅샷인데 단지 카탈로그는 계속 갱신되므로, 공고가 그때 뭐라고 했는지는 여기 남아야 한다.
 */
@Embeddable
@Getter
